...
class Packet(object):
    ...
    """
    def __init__(self, time, size, data, id, src="a", dst="z", flow_id=0):
        self.time = time
        self.size = size
        self.id = id
        self.src = src
...
class PacketGenerator(object):
    ...
    """
    def __init__(self, env, id,  adist, sdist, data_generator, initial_delay=0, finish=float("inf"), flow_id=0):
        self.id = id
        self.env = env
        self.adist = adist
        self.sdist = sdist
    ...
    def run(self):
        ...
        while self.env.now < self.finish:
            ...
            self.packets_sent += 1
            p = Packet(self.env.now, self.sdist(), self.data_generator(), self.packets_sent, src=self.id, flow_id=self.flow_id)
            self.out.put(p)
